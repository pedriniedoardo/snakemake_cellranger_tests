...
rule moveCellrangerSummaryCellTypeMultiRun:
    '''
    This is the rule to move the outputs of interest.
    '''
    input:
        cellranger_summary = rules.runCellrangerMultiRun.output.output
    output:
        summary = config["out_location"] + "web_summaries/merged/{sample_name}_web_summary_cell_types.html"
    # conda: config["env_bioinfo"]
    log:
        'logs/merged/{sample_name}/moveCellrangerSummaryCellTypeMultiRun.log'
    benchmark:
        'benchmarks/merged/{sample_name}/moveCellrangerSummaryCellTypeMultiRun.txt'
    resources:
        mem_mb = 250,
        cpus = 1
    threads: 1
    params:
        wd_summary = config["out_location"] + 'cellranger/merged/{sample_name}/outs/web_summary_cell_types.html',
    shell:
        '''
        # copy
        echo "copy summary <{wildcards.sample_name}>" >> {log}

rule runCellranger:
    ...
    params:
        ...
        ANNO_param = config['ANNO_param'],
        ANNO_flag = config['ANNO_flag'],
    ...
    shell:
        ...
        {params.ANNO_param}={params.ANNO_flag} \
        --localmem={params.RAM} \
...
rule moveCellrangerSummaryCellType:
    '''
    This is the rule to copy the cell deconvolution resutl in on folder.
    '''
    input:
        cellranger_summary = rules.runCellranger.output.output
    output:
        summary = config["out_location"] + "web_summaries/single/{sample_run}/{sample_name}_web_summary_cell_types.html"
    # conda: config["env_bioinfo"]
    log:
        'logs/single/{sample_run}/{sample_name}/moveCellrangerSummaryCellType.log'
    benchmark:
        'benchmarks/single/{sample_run}/{sample_name}/moveCellrangerSummaryCellType.txt'
    resources:
        mem_mb = 250,
        cpus = 1
    threads: 1
    params:
        wd_summary = config["out_location"] + 'cellranger/single/{sample_run}/{sample_name}/outs/web_summary_cell_types.html',
    shell:
        '''
        # copy
        echo "copy summary <{wildcards.sample_name}>" >> {log}
